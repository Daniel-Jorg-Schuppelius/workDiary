...
(function () {
    ...
    var confirmDialog = /** @type {HTMLDialogElement} */ (
        document.getElementById("action-confirm-dialog")
    );
    ...
    var toneAccents = {
        ...
        success: {
            header: "from-success/15 via-success/5 to-transparent",
            icon: "bg-success/15 text-success",
        },
        warning: {
            header: "from-warning/15 via-warning/5 to-transparent",
            icon: "bg-warning/15 text-warning",
        },
        error: {
            header: "from-error/15 via-error/5 to-transparent",
            icon: "bg-error/15 text-error",
        },
        info: {
            header: "from-info/15 via-info/5 to-transparent",
            icon: "bg-info/15 text-info",
        },
        ghost: {
            header: "from-base-300/40 via-base-200/30 to-transparent",
    ...
    Object.keys(toneAccents).forEach(function (k) {
        toneAccents[k].header.split(" ").forEach(function (c) {
            if (toneClasses.indexOf(c) < 0) toneClasses.push(c);
        });
        toneAccents[k].icon.split(" ").forEach(function (c) {
            if (toneClasses.indexOf(c) < 0) toneClasses.push(c);
        });
    });
    ...
    function applyTone(headerEl, iconWrapEl, tone) {
        ...
        accent.header.split(" ").forEach(function (c) {
            headerEl.classList.add(c);
        });
        accent.icon.split(" ").forEach(function (c) {
            iconWrapEl.classList.add(c);
        });
    }
    ...
    function renderIcon(el, value) {
        ...
        if (v !== "" && /^[a-z0-9_]+$/.test(v)) {
            setHtml(
                el,
                html`<span class="material-symbols-outlined">${v}</span>`,
            );
        } else {
            el.textContent = v;
        }
    ...
    function openConfirm(opts) {
        opts = opts || {};
        var tone =
            opts.tone || (opts.dangerous === false ? "primary" : "warning");
        if (confirmTitle) {
        ...
        if (confirmIcon) {
            renderIcon(
                confirmIcon,
                opts.icon || (opts.dangerous === false ? "help" : "warning"),
            );
        }
    ...
    function openNotify(opts) {
        ...
        if (notifyTitle) {
            notifyTitle.textContent =
                opts.title || defaultTitles[tone] || I.notifyInfo;
        }
        if (notifyMessage) {
    ...
    window.notifyAction = function (opts) {
        if (!notifyDialog) {
            try {
                ...
                window.alert(
                    (typeof opts === "object" && opts && opts.message) ||
                        String(opts || ""),
                );
            } catch (e) {
                /* ignore */
    ...
    if (confirmDialog && confirmSubmit) {
        ...
        document.addEventListener("click", function (event) {
            ...
            var ownerForm =
                trigger.form || (trigger.closest && trigger.closest("form"));
            ...
            if (
                ...
                (trigger.type === "submit" || !trigger.type) &&
                ownerForm
            ) {
            ...
            openConfirm({
                ...
                message:
                    trigger.getAttribute("data-confirm-message") || undefined,
    ...
    (function () {
        ...
        var sections = /** @type {NodeListOf<HTMLDetailsElement>} */ (
            document.querySelectorAll(
                "#app-sidebar details[data-sidebar-section-key]",
            )
        );
    ...
    (function () {
        ...
        var groups = /** @type {NodeListOf<HTMLDetailsElement>} */ (
            document.querySelectorAll(
                "#app-sidebar details[data-sidebar-subgroup-key]",
            )
        );
    ...
    if (sidebar) {
        ...
        sidebar.addEventListener("click", function (event) {
            var link = /** @type {HTMLElement} */ (event.target).closest("a");
