...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.getElementById('shift-dialog-form')?.addEventListener('submit', onShiftDialogSave);
    document.getElementById('shift-dialog-delete')?.addEventListener('click', onShiftDialogDelete);

    // Shift-type form: save
    document.getElementById('shift-type-form')?.addEventListener('submit', onShiftTypeSave);
});
...
function openShiftDialog ({ date = null, userId = null, shiftId = null, shift = null } = {}) {
    ...
    document.getElementById('shift-dialog-delete').classList.toggle('hidden', ! isEdit);
    document.getElementById('shift-dialog-status-row').classList.toggle('hidden', ! isEdit);
...
async function onShiftDialogSave (event) {
    ...
    const body = {
        ...
        shift_type_id: document.getElementById('shift-dialog-type').value    || null,
    ...
    if (isEdit) {
        body.status = document.getElementById('shift-dialog-status')?.value ?? undefined;
    }
...
window.shiftTypeOpenEdit = function (typeId, type) {
    ...
    document.getElementById('shift-type-start').value  = (type.default_start_time ?? '').slice(0, 5);
    document.getElementById('shift-type-end').value    = (type.default_end_time   ?? '').slice(0, 5);
    const active = document.getElementById('shift-type-active');
    if (active) active.checked = type.is_active ?? true;

    document.getElementById('shift-type-form-title').textContent = _t('Schichttyp bearbeiten');
...
window.shiftTypeResetForm = function () {
    ...
    document.getElementById('shift-type-form-title').textContent = _t('Neuen Schichttyp anlegen');
...
async function onShiftTypeSave (event) {
    ...
    const body = {
        ...
        default_start_time:  document.getElementById('shift-type-start').value || null,
        default_end_time:    document.getElementById('shift-type-end').value   || null,
...
function updateTypeRow (id, type) {
    ...
    if (colorSwatch) colorSwatch.style.backgroundColor = type.color ?? '#6b7280';
