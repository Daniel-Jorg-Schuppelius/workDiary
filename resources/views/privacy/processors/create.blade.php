...
@section('nav-title', __('Neuer Dienstleister'))
@section('content')
    <x-index-page :subtitle="__('Neuen Dienstleister bzw. Vertragspartner anlegen.')">
        <x-slot:actions>
        ...
        <x-card>
            <form method="post" action="{{ route('dataprotection.processors.store') }}" class="space-y-3">
                ...
                <x-icon-btn icon="add" tone="primary" type="submit" show-label>{{ __('Anlegen') }}</x-icon-btn>
            </form>
        </x-card>
    </x-index-page>
@endsection
