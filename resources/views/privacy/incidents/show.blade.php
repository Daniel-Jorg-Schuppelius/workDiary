...
@section('content')
    <div class="p-4 max-w-4xl space-y-4">
        <div class="flex items-center justify-between">
            <h1 class="text-xl font-semibold">{{ $incident->incident_number }} — {{ $incident->type->label() }}</h1>
            <span class="badge {{ $incident->isDeadlineBreached() ? 'badge-error' : 'badge-ghost' }}">{{ $incident->status->label() }}</span>
        </div>
        ...
        <div class="grid md:grid-cols-3 gap-4">
            <section class="card bg-base-200 p-4 md:col-span-2 text-sm space-y-2">
        ...
        <section class="card bg-base-200 p-4 space-y-2">
            <h2 class="font-semibold">{{ __('Maßnahmen') }}</h2>
            <ul class="space-y-1">
