...
@php
/**
 * @var \App\Models\Finance\CashRegister $register
 * @var \Illuminate\Pagination\LengthAwarePaginator<int, \App\Models\Finance\CashEntry> $entries
 * @var array<int, int> $reversedIds
 * @var float $balance
 * @var \Carbon\Carbon|null $lastClosing
 * @var \Illuminate\Database\Eloquent\Collection<int, \App\Models\Finance\CashDailyClosing> $closings
 */
@endphp
    ...
    <x-table scroll="flex" :pinRows="true" :zebra="true" size="sm">
        ...
        @forelse ($entries as $entry)
            ...
            <tr @class(['hover', 'opacity-60' => $isReversed])>
                ...
                <td class="text-right tabular-nums">{{ $entry->direction === \App\Models\Finance\CashEntry::DIRECTION_IN ? \CommonToolkit\Helper\Data\NumberHelper::toGermanFormat($entry->amount?->toFloat() ?? 0.0, 2, withThousandsSeparator: true) : '' }}</td>
                <td class="text-right tabular-nums">{{ $entry->direction === \App\Models\Finance\CashEntry::DIRECTION_OUT ? \CommonToolkit\Helper\Data\NumberHelper::toGermanFormat($entry->amount?->toFloat() ?? 0.0, 2, withThousandsSeparator: true) : '' }}</td>
                @if ($postingStates !== [])
