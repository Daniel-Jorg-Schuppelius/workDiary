{{--
  ...
  Filename     : _week_matrix.blade.php
                    ...
                    @foreach ($cellShifts as $shift)
                        ...
                        <div class="schedule-shift-badge mb-0.5 flex items-center gap-1 rounded px-1.5 py-0.5 text-xs font-semibold leading-tight shadow-xs
                            ...
                            <span class="ml-auto inline-flex items-center gap-0.5">
                                @if ($qualGap)
                                    <span class="inline-flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white/90 text-[0.6rem] font-bold text-error" aria-hidden="true" title="{{ __('schedule.qualification.missing') }}: {{ implode(', ', $qualGap) }}">Q</span>
                                @endif
                                @if ($compl)
                                    <span class="inline-flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white/90 text-[0.6rem] font-bold {{ $compl['severity'] === 'error' ? 'text-error' : 'text-warning' }}" aria-hidden="true">!</span>
                                @endif
                            </span>
                        </div>
                    @endforeach
