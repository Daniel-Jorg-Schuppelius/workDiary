...
@php
    ...
    $confidenceLabels = [
        ArticleDuplicateFinder::CONF_EXACT  => __('Eindeutig'),
        ArticleDuplicateFinder::CONF_LIKELY => __('Wahrscheinlich'),
        ArticleDuplicateFinder::CONF_FUZZY  => __('Möglich'),
    ];
...
@section('content')
<x-index-page :subtitle="__('Doppelte Artikel (z. B. aus Katalog-Adoption, CSV-Import und manueller Anlage) werden hier gegenübergestellt. Varianten wandern samt Bestandshistorie als Ganzes zum Ziel-Artikel — Bestände werden nie vermischt. Läuft eine Inventur oder ein Fertigungsauftrag, wird das Zusammenführen abgelehnt.')">
    <x-slot:actions>
        <form method="GET" action="{{ route('articles.duplicates.index') }}" class="flex items-center gap-2">
            <select name="confidence" class="select select-sm select-bordered" data-autosubmit>
                ...
                <option value="{{ ArticleDuplicateFinder::CONF_EXACT }}"  @selected($confidence === ArticleDuplicateFinder::CONF_EXACT)>{{ $confidenceLabels[ArticleDuplicateFinder::CONF_EXACT] }}</option>
                <option value="{{ ArticleDuplicateFinder::CONF_LIKELY }}" @selected($confidence === ArticleDuplicateFinder::CONF_LIKELY)>{{ $confidenceLabels[ArticleDuplicateFinder::CONF_LIKELY] }}</option>
                <option value="{{ ArticleDuplicateFinder::CONF_FUZZY }}"  @selected($confidence === ArticleDuplicateFinder::CONF_FUZZY)>{{ $confidenceLabels[ArticleDuplicateFinder::CONF_FUZZY] }}</option>
            </select>
        </form>
    </x-slot:actions>
    ...
    <div class="mb-4 rounded-box border border-base-300 bg-base-100 p-4 shadow-xs">
        <details @if (session('manual_open')) open @endif>
            <summary class="cursor-pointer text-sm font-medium">
                ...
                <span class="ml-1 text-base-content/50">{{ __('— zwei Artikel frei wählen (für Dubletten, die der Abgleich nicht erkennt)') }}</span>
            </summary>
            <form method="GET" action="{{ route('articles.duplicates.compare') }}"
                  class="mt-3 grid grid-cols-1 gap-3 md:grid-cols-[1fr_1fr_auto] md:items-end">
                <div class="fieldset">
                    <label class="fieldset-label" for="manual-target">
                        <span class="badge badge-xs badge-success mr-1">{{ __('Bleibt') }}</span>{{ __('Ziel-Artikel') }}
                    </label>
                    <select name="target" id="manual-target" required class="select select-bordered w-full">
                        <option value="">{{ __('— wählen —') }}</option>
                        @foreach ($articles as $article)
                            <option value="{{ $article->sqid }}">{{ $article->name }}@if ($article->number) ({{ $article->number }})@endif</option>
                        @endforeach
                    </select>
                </div>
                <div class="fieldset">
                    <label class="fieldset-label" for="manual-source">
                        <span class="badge badge-xs badge-error mr-1">{{ __('Wird gelöscht') }}</span>{{ __('Quell-Artikel') }}
                    </label>
        ...
        <div x-data="pairSelection"
            ...
            <div x-cloak x-show="hasSelection()"
                ...
                <div class="flex items-center gap-2">
                    ...
                    <form method="POST" action="{{ route('articles.duplicates.bulk-merge') }}"
                          data-confirm-dialog
                          data-confirm-message="{{ __('Alle ausgewählten Paare zusammenführen? Die jeweils markierten Quell-Artikel werden gelöscht — das kann nicht rückgängig gemacht werden.') }}"
            ...
            @foreach ($candidates as $pair)
                ...
                <div class="rounded-box border border-base-300 bg-base-100 p-4 shadow-xs">
                    <div class="mb-3 flex flex-wrap items-center gap-2">
                        ...
                        @php
                            $confBadge = match ($conf) {
                                ArticleDuplicateFinder::CONF_EXACT => 'badge-error',
                                ArticleDuplicateFinder::CONF_LIKELY => 'badge-warning',
                                default => 'badge-ghost',
                            };
                        @endphp
                    ...
                    <div class="mt-3 flex flex-wrap justify-end gap-2">
                        <a href="{{ route('articles.duplicates.compare', ['target' => $target->sqid, 'source' => $source->sqid]) }}"
                        ...
                        <form method="POST" action="{{ route('articles.duplicates.merge') }}"
                              data-confirm-dialog
                              data-confirm-message="{{ __('Richtung tauschen: „:source“ in „:target“ zusammenführen? Der Quell-Artikel wird gelöscht.', ['source' => $target->name, 'target' => $source->name]) }}"
                        ...
                        <form method="POST" action="{{ route('articles.duplicates.dismiss') }}">
                            @csrf
                            <input type="hidden" name="source" value="{{ $source->sqid }}">
                            <input type="hidden" name="target" value="{{ $target->sqid }}">
