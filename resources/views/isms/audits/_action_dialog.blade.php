    ...
    <x-form-group :legend="__('isms.group.corrective_action')" icon="build_circle" tone="primary" cols="2">
        ...
        <x-select-field name="owner_user_id" :label="__('isms.field.owner')">
            ...
            @foreach ($owners as $owner)
                <option value="{{ $owner->sqid }}" @selected((string) old('owner_user_id', \App\Support\Sqid::encode(\App\Models\Platform\User::class, $action?->owner_user_id)) === $owner->sqid)>{{ $owner->name }}</option>
            @endforeach
        </x-select-field>
