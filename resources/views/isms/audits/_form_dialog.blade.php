    ...
    <x-form-group :legend="__('isms.group.auditors')" icon="group" tone="warning" cols="2">
        <x-select-field name="lead_auditor_user_id" :label="__('isms.field.lead_auditor')">
            ...
            @foreach ($auditorOptions as $auditor)
                <option value="{{ $auditor->sqid }}" @selected((string) old('lead_auditor_user_id', \App\Support\Sqid::encode(\App\Models\Platform\User::class, $audit?->lead_auditor_user_id)) === $auditor->sqid)>{{ $auditor->name }}</option>
            @endforeach
        </x-select-field>
