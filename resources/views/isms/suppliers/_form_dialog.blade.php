    ...
    <x-form-group :legend="__('isms.group.supplier')" icon="handshake" tone="primary" cols="2">
        ...
        <x-select-field name="owner_user_id" :label="__('isms.field.owner')">
                ...
                @foreach ($owners as $owner)
                    <option value="{{ $owner->sqid }}" @selected((string) old('owner_user_id', \App\Support\Sqid::encode(\App\Models\Platform\User::class, $assessment?->owner_user_id)) === $owner->sqid)>{{ $owner->name }}</option>
                @endforeach
        </x-select-field>
    </x-form-group>
