...
<x-card :title="__('Finanzen & Reisen') . ' · ' . ($month['label'] ?? '')">
    <div class="grid gap-3 sm:grid-cols-2">
        <div class="rounded-box border border-base-300 p-3">
            ...
            <p class="mt-1 text-xl font-semibold">{{ number_format((float) ($month['expenses_submitted_gross'] ?? 0), 2, ',', '.') }} €</p>
            <p class="mt-1 text-xs text-base-content/60">
                {{ __('Erstattet') }}: {{ number_format((float) ($month['expenses_reimbursed_gross'] ?? 0), 2, ',', '.') }} €
            </p>
        </div>
        <div class="rounded-box border border-base-300 p-3">
    ...
    <div class="mt-3 rounded-box border border-base-300 p-3">
        ...
        <p class="mt-1 text-sm">
            ...
            ·
            <span class="opacity-70">{{ rtrim(rtrim(number_format((float) ($vacation['approved_days_this_year'] ?? 0), 1, ',', '.'), '0'), ',') }}</span> {{ __('Tage genehmigt') }}
        </p>
    </div>
