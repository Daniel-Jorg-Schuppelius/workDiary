...
@section('content')
    <div class="mx-auto flex h-[calc(100dvh-11rem)] w-full max-w-3xl flex-col gap-4">
        ...
        <article>
            <div class="flex flex-wrap items-center justify-between gap-4 mb-6">
                ...
                <div class="flex gap-2">
                    @can('update', $diary)
                        <a href="{{ route('diary.edit', $diary) }}" class="btn btn-ghost btn-sm">{{ __('Bearbeiten') }}</a>
                    @endcan
                    @can('delete', $diary)
                        <form
                            method="POST"
        ...
        {{-- Legacy-Vergleich (wenn verfügbar) --}}
        @if ($legacyEntry)
        ...
        <div class="text-center">
            <a href="{{ route('diary.index') }}" class="text-sm text-primary hover:opacity-80 transition">← Zurück zur Liste</a>
        </div>
        </div>
        </div>
    </div>
