...
<head>
    ...
    <title>{{ __('Wartungsarbeiten') }} – {{ config('app.name', 'WorkDiary') }}</title>
    <style>
    ...
    <div class="flex min-h-screen items-center justify-center px-4">
        <div class="w-full max-w-lg rounded-3xl border border-base-300 bg-base-100 p-8 text-center shadow-lg">
            ...
            <div class="mb-4 inline-flex h-16 w-16 items-center justify-center rounded-full bg-warning/15 text-warning">
                <span class="material-symbols-outlined" style="font-size: 2rem; font-variation-settings: 'FILL' 1, 'wght' 500;">engineering</span>
            </div>
            ...
            @if (($until ?? null) instanceof \Carbon\CarbonInterface)
                <p class="mt-3 text-xs text-base-content/60">
                    {{ __('Voraussichtlich wieder verfügbar: :at', ['at' => $until->translatedFormat('d.m.Y H:i')]) }}
                </p>
            @endif
            <x-button-group center class="mt-6">
                <x-button href="{{ url('/') }}" tone="primary" size="sm" class="gap-1" icon="refresh">{{ __('Erneut versuchen') }}</x-button>
            </x-button-group>
        </div>
    </div>
</body>
