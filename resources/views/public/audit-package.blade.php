...
<main class="mx-auto max-w-5xl p-4 space-y-4">
    <div class="rounded-box bg-base-100 p-6 shadow">
        <div class="mb-1 text-xs uppercase tracking-wide text-base-content/60">{{ __('Auditpaket (finalisiert)') }}</div>
        <h1 class="text-xl font-semibold">{{ $meta['title'] ?? $package->title }}</h1>
        <dl class="mt-4 grid gap-x-8 gap-y-1 text-sm sm:grid-cols-2">
            <div class="flex justify-between gap-4"><dt class="text-base-content/60">{{ __('Organisation') }}</dt><dd>{{ $meta['organization'] ?? '—' }}</dd></div>
            <div class="flex justify-between gap-4"><dt class="text-base-content/60">{{ __('Geltungsbereich') }}</dt><dd>{{ $meta['scope'] ?? '—' }}</dd></div>
            <div class="flex justify-between gap-4"><dt class="text-base-content/60">{{ __('Norm') }}</dt><dd>{{ $meta['norm_filter'] ?? '—' }}</dd></div>
            <div class="flex justify-between gap-4"><dt class="text-base-content/60">{{ __('Paket-Nr.') }}</dt><dd class="font-mono">{{ $meta['package_no'] ?? $package->displayNo() }}</dd></div>
            {{-- Berichtsstichtag und Datenstand sind bewusst getrennt: der eine
                 ist dokumentiert, der andere der Zeitpunkt des Einfrierens. --}}
            <div class="flex justify-between gap-4"><dt class="text-base-content/60">{{ __('Berichtsstichtag') }}</dt><dd>{{ $meta['as_of_date'] ?? '—' }}</dd></div>
            <div class="flex justify-between gap-4"><dt class="text-base-content/60">{{ __('Datenstand (eingefroren)') }}</dt><dd>{{ $meta['data_captured_at'] ?? '—' }}</dd></div>
        </dl>
    ...
    @foreach ($sections as $key => $label)
        ...
        <div class="rounded-box bg-base-100 p-6 shadow">
            <h2 class="mb-1 font-semibold">{{ $label }} <span class="text-sm font-normal text-base-content/60">({{ count($rows) }})</span></h2>
            @if ($rows === [])
                <p class="text-sm text-base-content/60">{{ __('Keine Einträge in diesem Abschnitt.') }}</p>
            @else
                <div class="space-y-3">
                    @foreach ($rows as $row)
                            ...
                            @foreach ((array) $row as $field => $value)
                                @if (is_scalar($value) || $value === null)
                                    <div class="flex justify-between gap-3">
                                        <dt class="font-mono text-xs text-base-content/60">{{ $field }}</dt>
                                ...
                                @elseif (is_array($value) && $value !== [])
                                    <div class="sm:col-span-2">
                                        <dt class="font-mono text-xs text-base-content/60">{{ $field }} ({{ count($value) }})</dt>
                                                ...
                                                <div class="rounded bg-base-200/60 px-2 py-1 text-xs">
                                                    @foreach ((array) $sub as $subField => $subValue)
                                                        @if (is_scalar($subValue) || $subValue === null)
                                                            <span class="mr-3"><span class="font-mono text-base-content/50">{{ $subField }}:</span> {{ $scalar($subValue) !== '' ? $scalar($subValue) : '—' }}</span>
                                                        @endif
                                                    @endforeach
                                                </div>
                                            @endforeach
    ...
    <p class="pb-6 text-center text-xs text-base-content/50">
