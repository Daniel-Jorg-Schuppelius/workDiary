@extends('layouts.app')
@section('title', 'Callcenter Login — ' . config('app.name', 'WorkDiary'))
@section('nav-title', 'Callcenter Login')
...
@section('content')
    <div class="mx-auto flex h-[calc(100dvh-11rem)] w-full max-w-md flex-col">
        <div class="min-h-0 flex-1 overflow-hidden rounded-box border border-base-300 bg-base-100">
            ...
            <h2 class="mb-3 text-base font-bold">Notdienstplan Login</h2>
            <form method="POST" action="{{ route('legacy.callcenter.login.submit') }}" class="space-y-4">
                @csrf
                <div>
                    <label for="username" class="label text-sm font-semibold pb-1">Nutzer</label>
                ...
                <div>
                    <label for="password" class="label text-sm font-semibold pb-1">Passwort</label>
