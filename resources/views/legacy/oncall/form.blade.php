@extends('layouts.app')
@section('title', ($isEdit ? 'Bereitschaft bearbeiten' : 'Bereitschaft neu') . ' — WorkDiary')
@section('nav-title', $isEdit ? 'Bereitschaft bearbeiten' : 'Bereitschaft neu')
...
@section('content')
    <div class="mx-auto flex h-[calc(100dvh-11rem)] w-full max-w-xl flex-col">
        <div class="min-h-0 flex-1 overflow-hidden rounded-box border border-base-300 bg-base-100">
            ...
            <form method="POST" action="{{ $isEdit ? route('legacy.oncall.update', $item) : route('legacy.oncall.store') }}" class="space-y-5">
                @csrf
                @if ($isEdit)
                    @method('PUT')
                @endif
                ...
                <div>
                    <label for="user" class="label text-sm font-semibold pb-1">Mitarbeiter</label>
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                    <div>
                        <label for="von" class="label text-sm font-semibold pb-1">Von</label>
                    ...
                    <div>
                        <label for="bis" class="label text-sm font-semibold pb-1">Bis</label>
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <a href="{{ route('legacy.oncall.index') }}" class="btn btn-ghost btn-sm">Abbrechen</a>
                </div>
            </form>
            </div>
        </div>
