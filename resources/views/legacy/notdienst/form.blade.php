@extends('layouts.app')
@section('title', ($isEdit ? 'Notdienst bearbeiten' : 'Notdienst neu') . ' — WorkDiary')
@section('nav-title', $isEdit ? 'Notdienst bearbeiten' : 'Notdienst neu')
        ...
        <div class="min-h-0 flex-1 overflow-hidden rounded-box border border-base-300 bg-base-100">
            ...
            <form method="POST" action="{{ $isEdit ? route('legacy.notdienst.update', $item) : route('legacy.notdienst.store') }}" class="space-y-5">
                ...
                <div class="flex gap-2 pt-1">
                    ...
                    <a href="{{ route('legacy.notdienst.index') }}" class="btn btn-ghost btn-sm">Abbrechen</a>
                </div>
            </form>
            </div>
        </div>
