...
Auf dieser Seite verwalten Sie die Anbindung an den Identitätsanbieter
Ihrer Organisation: SCIM-Provisionierung, OIDC-Single-Sign-on und
SAML 2.0. Das Modul ist Teil des Enterprise-Plans.

**SCIM-Provisionierung:** Ihr Identitätsanbieter (Entra ID, Keycloak,
Okta …) legt Konten an, aktualisiert und deaktiviert sie über den
SCIM-Endpunkt. Dafür stellen Sie ein Bearer-Token aus — der Klartext
...
Rollen; SCIM-Gruppen können Sie bewusst einem Team zuordnen.

**OIDC-Single-Sign-on:** Hinterlegen Sie Issuer, Client-ID und
Client-Secret aus der App-Registrierung Ihres Identitätsanbieters
und registrieren Sie dort die angezeigte Callback-URL. Mitarbeitende
...
**SAML 2.0:** Hinterlegen Sie Entity-ID, SSO-URL und Signaturzertifikat des
Identitätsanbieters; die SP-Metadata-URL stellen Sie dem IdP bereit.
Antworten müssen signierte Assertions tragen; vom IdP initiierte
(unaufgeforderte) Anmeldungen werden abgelehnt. Für die
Zertifikatsrotation können Sie ein Folgezertifikat parallel hinterlegen.
...
Legen Sie vorher mindestens ein Break-Glass-Konto fest: ein nicht
