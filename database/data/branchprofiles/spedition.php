<?php
/*
 * Created on   : Sun May 24 2026
 * Author       : Daniel Jörg Schuppelius
 * Author Uri   : https://schuppelius.org
 * Filename     : spedition.php
 * License      : AGPL-3.0-or-later
 * License Uri  : https://www.gnu.org/licenses/agpl-3.0.html
 */

use App\Enums\Classification\{ClassificationRequirementPhase, ClassificationRequirementSeverity};

return [
    'code' => 'spedition',
    'label' => 'Spedition und Transportlogistik',
    'description' => 'Spedition und Transportlogistik: Transportauftrag, Disposition, Beladung, Zustellung, Lenk- und Ruhezeiten, Ladungssicherung und Schäden.',
    'version' => 2,
    // Feature 081 (MVP-373): empfohlener Funktionsumfang — als vorausgewählte
    // Checkliste auf der Seite „Funktionsumfang“, nie still angewendet.
    'modules_recommended' => [
        'module.planung',
        'module.spesen',
        'module.vertrieb',
        'module.documents',
        'module.forms',
        'module.knowledge',
        'module.auswertungen_team',
        'module.fuhrpark',
        'module.standorterfassung',
        'module.lager',
        'module.versand',
    ],
    'classifications' => [
        'entry_type' => [
            ['code' => 'transportauftrag', 'label' => 'Transportauftrag'],
            ['code' => 'disposition', 'label' => 'Disposition'],
            ['code' => 'abholung', 'label' => 'Abholung'],
            ['code' => 'beladung', 'label' => 'Beladung'],
            ['code' => 'umschlag', 'label' => 'Umschlag'],
            ['code' => 'transport', 'label' => 'Transport'],
            ['code' => 'zustellung', 'label' => 'Zustellung'],
            ['code' => 'rueckladung', 'label' => 'Rueckladung'],
            ['code' => 'wartezeit', 'label' => 'Wartezeit'],
            ['code' => 'schaden', 'label' => 'Schaden'],
            ['code' => 'reklamation', 'label' => 'Reklamation'],
            ['code' => 'nachkalkulation', 'label' => 'Nachkalkulation'],
        ],
        'activity' => [
            ['code' => 'planen', 'label' => 'Planen'],
            ['code' => 'avisieren', 'label' => 'Avisieren'],
            ['code' => 'laden', 'label' => 'Laden'],
            ['code' => 'sichern', 'label' => 'Sichern'],
            ['code' => 'fahren', 'label' => 'Fahren'],
            ['code' => 'umladen', 'label' => 'Umladen'],
            ['code' => 'entladen', 'label' => 'Entladen'],
            ['code' => 'scannen', 'label' => 'Scannen'],
            ['code' => 'dokumentieren', 'label' => 'Dokumentieren'],
            ['code' => 'palettenTauschen', 'label' => 'Palettentausch'],
        ],
        'defect_type' => [
            ['code' => 'lieferverzug', 'label' => 'Lieferverzug'],
            ['code' => 'transportschaden', 'label' => 'Transportschaden'],
            ['code' => 'fehlmenge', 'label' => 'Fehlmenge'],
            ['code' => 'falscheWare', 'label' => 'Falsche Ware'],
            ['code' => 'temperaturAbweichend', 'label' => 'Temperatur abweichend'],
            ['code' => 'annahmeVerweigert', 'label' => 'Annahme verweigert'],
            ['code' => 'dokumentFehlt', 'label' => 'Dokument fehlt'],
            ['code' => 'lademittelDifferenz', 'label' => 'Lademitteldifferenz'],
        ],
        'root_cause' => [
            ['code' => 'verkehr', 'label' => 'Verkehr'],
            ['code' => 'verspaeteteBereitstellung', 'label' => 'Verspaetete Bereitstellung'],
            ['code' => 'rampenstau', 'label' => 'Rampenstau'],
            ['code' => 'falscheAdresse', 'label' => 'Falsche Adresse'],
            ['code' => 'fahrzeugDefekt', 'label' => 'Fahrzeugdefekt'],
            ['code' => 'personalEngpass', 'label' => 'Personalengpass'],
            ['code' => 'wetter', 'label' => 'Wetter'],
            ['code' => 'kundenfehler', 'label' => 'Kundenfehler'],
            ['code' => 'subunternehmer', 'label' => 'Subunternehmer'],
        ],
        'result' => [
            ['code' => 'zugestellt', 'label' => 'Zugestellt'],
            ['code' => 'teilZugestellt', 'label' => 'Teilweise zugestellt'],
            ['code' => 'nichtZugestellt', 'label' => 'Nicht zugestellt'],
            ['code' => 'retourniert', 'label' => 'Retourniert'],
            ['code' => 'umdisponiert', 'label' => 'Umdisponiert'],
            ['code' => 'eskaliert', 'label' => 'Eskaliert'],
            ['code' => 'schadenGemeldet', 'label' => 'Schaden gemeldet'],
            ['code' => 'abgerechnet', 'label' => 'Abgerechnet'],
        ],
        'product_group' => [
            ['code' => 'stueckgut', 'label' => 'Stueckgut'],
            ['code' => 'teilpartie', 'label' => 'Teilpartie'],
            ['code' => 'komplettladung', 'label' => 'Komplettladung'],
            ['code' => 'express', 'label' => 'Express'],
            ['code' => 'kuehlgut', 'label' => 'Kuehlgut'],
            ['code' => 'gefahrgut', 'label' => 'Gefahrgut'],
            ['code' => 'sperrgut', 'label' => 'Sperrgut'],
            ['code' => 'palettenware', 'label' => 'Palettenware'],
            ['code' => 'retouren', 'label' => 'Retouren'],
            ['code' => 'lagerware', 'label' => 'Lagerware'],
        ],
        'dienstmittel_type' => [
            ['code' => 'sattelzug', 'label' => 'Sattelzug'],
            ['code' => 'wechselbruecke', 'label' => 'Wechselbruecke'],
            ['code' => 'sprinter', 'label' => 'Sprinter'],
            ['code' => 'kuehlfahrzeug', 'label' => 'Kuehlfahrzeug'],
            ['code' => 'anhaenger', 'label' => 'Anhaenger'],
            ['code' => 'auflieger', 'label' => 'Auflieger'],
            ['code' => 'stapler', 'label' => 'Stapler'],
            ['code' => 'hubwagen', 'label' => 'Hubwagen'],
            ['code' => 'scanner', 'label' => 'Scanner'],
            ['code' => 'spanngurt', 'label' => 'Spanngurt'],
        ],
        // Genehmigungsarten (Auswahl/Filter je Eintrag). Status, Frist und
        // Nachweis werden im Genehmigungs-Register (Permit) geführt.
        'permit_type' => [
            ['code' => 'grossraumtransport', 'label' => 'Großraumtransport'],
            ['code' => 'schwertransport', 'label' => 'Schwertransport'],
            ['code' => 'gefahrgut_adr', 'label' => 'Gefahrgut / ADR'],
            ['code' => 'kabotage', 'label' => 'Kabotage'],
            ['code' => 'gewichtsausnahme', 'label' => 'Gewichts-/Maßausnahme'],
            ['code' => 'sondernutzung', 'label' => 'Sondernutzung Straße'],
        ],
    ],
    'classification_requirements' => [
        [
            'entry_type_code' => 'transportauftrag',
            'required_domain' => 'permit_type',
            'enforce_phase' => ClassificationRequirementPhase::OnCreate->value,
            'severity' => ClassificationRequirementSeverity::Soft->value,
            'allow_multi' => true,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'transportauftrag',
            'required_domain' => 'product_group',
            'enforce_phase' => ClassificationRequirementPhase::OnCreate->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'transportauftrag',
            'required_domain' => 'dienstmittel_type',
            'enforce_phase' => ClassificationRequirementPhase::OnCreate->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'disposition',
            'required_domain' => 'result',
            'enforce_phase' => ClassificationRequirementPhase::BeforeComplete->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'abholung',
            'required_domain' => 'result',
            'enforce_phase' => ClassificationRequirementPhase::BeforeComplete->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'beladung',
            'required_domain' => 'result',
            'enforce_phase' => ClassificationRequirementPhase::BeforeComplete->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'transport',
            'required_domain' => 'dienstmittel_type',
            'enforce_phase' => ClassificationRequirementPhase::OnCreate->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'zustellung',
            'required_domain' => 'result',
            'enforce_phase' => ClassificationRequirementPhase::BeforeComplete->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'schaden',
            'required_domain' => 'defect_type',
            'enforce_phase' => ClassificationRequirementPhase::OnCreate->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'schaden',
            'required_domain' => 'result',
            'enforce_phase' => ClassificationRequirementPhase::BeforeComplete->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'wartezeit',
            'required_domain' => 'root_cause',
            'enforce_phase' => ClassificationRequirementPhase::BeforeComplete->value,
            'severity' => ClassificationRequirementSeverity::Hard->value,
            'min_count' => 1,
        ],
        [
            'entry_type_code' => 'reklamation',
            'required_domain' => 'defect_type',
            'enforce_phase' => ClassificationRequirementPhase::BeforeComplete->value,
            'severity' => ClassificationRequirementSeverity::Soft->value,
            'min_count' => 1,
        ],
    ],
    'procedure_templates' => [
        [
            'code' => 'SP_LADUNGSSICHERUNG',
            'name' => 'Ladungssicherung',
            'domain' => 'spedition',
            'risk_level' => 'high',
            'description' => 'Prüfung der Ladungssicherung vor Fahrtantritt mit Freigabe.',
            'steps' => [
                ['code' => 'ladung', 'step_type' => 'confirm', 'label' => 'Ladung/Verteilung prüfen'],
                ['code' => 'sicherungsmittel', 'step_type' => 'confirm', 'label' => 'Zurrgurte/Sicherungsmittel prüfen'],
                ['code' => 'gewicht', 'step_type' => 'confirm', 'label' => 'Achslasten/Gesamtgewicht prüfen'],
                ['code' => 'foto', 'step_type' => 'photo', 'label' => 'Ladungssicherung dokumentieren', 'required' => false, 'blocking' => false, 'requires_proof_type' => 'photo'],
                ['code' => 'freigabe', 'step_type' => 'freigabe', 'label' => 'Fahrt freigeben'],
            ],
        ],
        [
            'code' => 'SP_ZUSTELLUNG_POD',
            'name' => 'Zustellung (POD)',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Zustellung beim Empfänger mit Ablieferquittung (Proof of Delivery).',
            'steps' => [
                ['code' => 'empfaenger', 'step_type' => 'confirm', 'label' => 'Empfänger/Abladestelle prüfen'],
                ['code' => 'uebergabe', 'step_type' => 'confirm', 'label' => 'Ware übergeben / Vollständigkeit prüfen'],
                ['code' => 'pod', 'step_type' => 'signature', 'label' => 'Ablieferquittung (POD) unterschreiben lassen', 'requires_proof_type' => 'signature'],
                ['code' => 'foto', 'step_type' => 'photo', 'label' => 'Abstellort/Übergabe fotografieren', 'required' => false, 'blocking' => false, 'requires_proof_type' => 'photo'],
            ],
        ],
        [
            'code' => 'SP_SCHADENSMELDUNG',
            'name' => 'Schadensmeldung',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Aufnahme eines Transportschadens mit Foto-Nachweis.',
            'steps' => [
                ['code' => 'schaden', 'step_type' => 'text', 'label' => 'Schaden beschreiben'],
                ['code' => 'fotos', 'step_type' => 'photo', 'label' => 'Schaden fotografieren', 'requires_proof_type' => 'photo'],
                ['code' => 'meldung', 'step_type' => 'confirm', 'label' => 'Disposition/Versicherung informieren'],
            ],
        ],
        [
            'code' => 'SP_TRANSPORTAUFTRAG',
            'name' => 'Transportauftrag annehmen',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Auftragsannahme mit Ladungsart, Gefahrgutprüfung, Maßen/Gewicht und Bestätigung.',
            'steps' => [
                ['code' => 'auftragsdaten', 'step_type' => 'text', 'label' => 'Absender, Empfänger und Termine erfassen'],
                ['code' => 'gut', 'step_type' => 'choice', 'label' => 'Ladungsart (Stückgut, Palette, Gefahrgut, Kühlgut)'],
                ['code' => 'gefahrgut', 'step_type' => 'confirm', 'label' => 'Gefahrgutpapiere/UN-Nummer geprüft', 'required' => false, 'blocking' => false],
                ['code' => 'masse', 'step_type' => 'confirm', 'label' => 'Maße, Gewicht und Ladehilfsmittel geprüft'],
                ['code' => 'preis', 'step_type' => 'confirm', 'label' => 'Fracht und Zuschläge bestätigt'],
                ['code' => 'bestaetigung', 'step_type' => 'file', 'label' => 'Auftragsbestätigung ablegen', 'required' => false, 'blocking' => false, 'requires_proof_type' => 'file'],
            ],
        ],
        [
            'code' => 'SP_DISPOSITION_TOUR',
            'name' => 'Tourendisposition',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Tour disponieren: Fahrzeug, Fahrer, Stopp-Reihenfolge, Papiere und Freigabe.',
            'steps' => [
                ['code' => 'fahrzeug', 'step_type' => 'dienstmittel', 'label' => 'Fahrzeug/Anhänger zuweisen'],
                ['code' => 'fahrer', 'step_type' => 'confirm', 'label' => 'Fahrer mit ausreichender Lenkzeit und Qualifikation eingeteilt'],
                ['code' => 'reihenfolge', 'step_type' => 'confirm', 'label' => 'Stopp-Reihenfolge und Zeitfenster geplant'],
                ['code' => 'papiere', 'step_type' => 'confirm', 'label' => 'Frachtpapiere/Lieferscheine bereitgestellt'],
                ['code' => 'freigabe', 'step_type' => 'freigabe', 'label' => 'Tour freigeben'],
            ],
        ],
        [
            'code' => 'SP_ABHOLUNG',
            'name' => 'Abholung beim Versender',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Abholung mit Identifikation, Zustandsprüfung, Packstückzählung und Übernahmequittung.',
            'steps' => [
                ['code' => 'identitaet', 'step_type' => 'confirm', 'label' => 'Absender und Sendung identifizieren'],
                ['code' => 'zustand', 'step_type' => 'choice', 'label' => 'Zustand/Verpackung prüfen'],
                ['code' => 'menge', 'step_type' => 'number', 'label' => 'Anzahl Packstücke'],
                ['code' => 'foto', 'step_type' => 'photo', 'label' => 'Sendung dokumentieren', 'required' => false, 'blocking' => false, 'requires_proof_type' => 'photo'],
                ['code' => 'uebernahme', 'step_type' => 'signature', 'label' => 'Übernahmequittung unterschreiben lassen', 'requires_proof_type' => 'signature'],
            ],
        ],
        [
            'code' => 'SP_TRANSPORTEREIGNIS',
            'name' => 'Transportereignis melden',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Ereignis unterwegs (Verspätung, Panne, Unfall, Kontrolle, Sperrung) melden und abstimmen.',
            'steps' => [
                ['code' => 'ereignis', 'step_type' => 'choice', 'label' => 'Ereignis (Verspätung, Panne, Unfall, Kontrolle, Sperrung)'],
                ['code' => 'beschreibung', 'step_type' => 'text', 'label' => 'Ereignis beschreiben'],
                ['code' => 'zeitort', 'step_type' => 'text', 'label' => 'Zeit und Ort festhalten'],
                ['code' => 'massnahme', 'step_type' => 'confirm', 'label' => 'Disposition informiert und Maßnahme abgestimmt'],
                ['code' => 'foto', 'step_type' => 'photo', 'label' => 'Situation dokumentieren', 'required' => false, 'blocking' => false, 'requires_proof_type' => 'photo'],
            ],
        ],
        [
            'code' => 'SP_PALETTENTAUSCH',
            'name' => 'Palettentausch',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Tausch von Ladehilfsmitteln mit Mengen, Qualitätsklasse und Palettenschein.',
            'steps' => [
                ['code' => 'abgegeben', 'step_type' => 'number', 'label' => 'Abgegebene Europaletten'],
                ['code' => 'erhalten', 'step_type' => 'number', 'label' => 'Erhaltene Europaletten'],
                ['code' => 'qualitaet', 'step_type' => 'choice', 'label' => 'Qualitätsklasse der Paletten'],
                ['code' => 'beleg', 'step_type' => 'signature', 'label' => 'Palettenschein unterschreiben lassen', 'requires_proof_type' => 'signature'],
                ['code' => 'foto', 'step_type' => 'photo', 'label' => 'Paletten dokumentieren', 'required' => false, 'blocking' => false, 'requires_proof_type' => 'photo'],
            ],
        ],
        [
            'code' => 'SP_WARTEZEIT_DOKU',
            'name' => 'Wartezeit dokumentieren',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Standzeit an Lade-/Entladestelle mit Zeiten, Grund und Bestätigung.',
            'steps' => [
                ['code' => 'ankunft', 'step_type' => 'text', 'label' => 'Ankunftszeit festhalten'],
                ['code' => 'beginn', 'step_type' => 'text', 'label' => 'Beginn der Be-/Entladung festhalten'],
                ['code' => 'grund', 'step_type' => 'choice', 'label' => 'Grund der Wartezeit'],
                ['code' => 'bestaetigung', 'step_type' => 'signature', 'label' => 'Wartezeit bestätigen lassen', 'required' => false, 'blocking' => false, 'requires_proof_type' => 'signature'],
                ['code' => 'meldung', 'step_type' => 'confirm', 'label' => 'Disposition informiert'],
            ],
        ],
        [
            'code' => 'SP_NACHKALKULATION',
            'name' => 'Tour nachkalkulieren',
            'domain' => 'spedition',
            'risk_level' => 'normal',
            'description' => 'Nachkalkulation der Tour: Kilometer, Zeiten, Zusatzkosten und Abgleich mit der Kalkulation.',
            'steps' => [
                ['code' => 'km', 'step_type' => 'number', 'label' => 'Gefahrene Kilometer'],
                ['code' => 'zeiten', 'step_type' => 'text', 'label' => 'Fahr- und Standzeiten erfassen'],
                ['code' => 'kosten', 'step_type' => 'text', 'label' => 'Maut und Zusatzkosten erfassen'],
                ['code' => 'abgleich', 'step_type' => 'choice', 'label' => 'Ergebnis gegen Kalkulation (im Plan, Abweichung)'],
                ['code' => 'massnahme', 'step_type' => 'text', 'label' => 'Maßnahme bei Abweichung', 'required' => false, 'blocking' => false],
            ],
        ],
    ],
    // 'protocol_templates': Codes aus database/data/protocol_templates.php (MVP-902).
    // 'asset_categories' installiert der BranchProfileInstaller nicht (Kategorien
    // stammen aus config('asset_categories')); sie dienen als Branchen-Taxonomie.
    'protocol_templates' => [
        ['code' => 'SP_ABHOLBELEG'],
        ['code' => 'SP_LADUNGSSICHERUNG'],
        ['code' => 'SP_ZUSTELLNACHWEIS'],
        ['code' => 'SP_PALETTENSCHEIN'],
        ['code' => 'SP_SCHADENSPROTOKOLL'],
        ['code' => 'SP_WARTEZEITNACHWEIS'],
        ['code' => 'SP_TOURBERICHT'],
    ],
    'asset_categories' => [
        'sattelzug',
        'zugmaschine',
        'auflieger',
        'wechselbruecke',
        'anhaenger',
        'sprinter',
        'kuehlfahrzeug',
        'stapler',
        'hubwagen',
        'scanner',
        'telematikgeraet',
        'spanngurt',
    ],
    'tags_seed' => [
        '#express',
        '#kuehlgut',
        '#gefahrgut',
        '#adr',
        '#rampe',
        '#wartezeit',
        '#palettentausch',
        '#schaden',
        '#retoure',
        '#subunternehmer',
    ],

    // Schulungsvorschläge (Feature 145): Pflichtschulungen im Güterverkehr.
    'training_suggestions' => [
        ['code' => 'ladungssicherung', 'title' => 'Ladungssicherung', 'legal_basis' => 'VDI 2700 / § 22 StVO', 'validity_months' => 24, 'duration_minutes' => 480, 'roles' => ['aussendienst']],
        ['code' => 'bkrfqg-weiterbildung', 'title' => 'Berufskraftfahrer-Weiterbildung', 'legal_basis' => 'BKrFQG § 5', 'provider_kind' => 'external', 'validity_months' => 60, 'duration_minutes' => 2100, 'lead_days' => 90, 'roles' => ['aussendienst']],
        ['code' => 'unterweisung-arbschg', 'title' => 'Jährliche Unterweisung Arbeitssicherheit', 'legal_basis' => '§ 12 ArbSchG / DGUV V1 § 4', 'validity_months' => 12, 'duration_minutes' => 60, 'roles' => ['aussendienst', 'teamleitung']],
    ],
];
