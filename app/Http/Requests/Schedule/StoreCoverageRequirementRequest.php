...
class StoreCoverageRequirementRequest extends FormRequest {
    ...
    protected array $sqidFields = [
        'shift_type_id' => \App\Models\Schedule\ShiftType::class,
        // Checkboxen posten Sqids — elementweise dekodieren.
        'required_qualification_ids' => \App\Models\Hr\Qualification::class,
    ];
    ...
    public function validationData(): array {
        ...
        if (is_array($minima)) {
            ...
            foreach ($minima as $key => $count) {
                ...
                $id = is_string($key)
                    ? \App\Support\Sqid::decodeOrNumeric(\App\Models\Hr\Qualification::class, $key)
                    : (int) $key;
                if ($id !== null && (int) $id > 0) {
    ...
    public function withValidator(\Illuminate\Validation\Validator $validator): void {
        $validator->after(function (\Illuminate\Validation\Validator $v): void {
            ...
            $known = \App\Models\Hr\Qualification::query()->whereIn('id', $ids)->count();
            if ($known !== count($ids)) {
