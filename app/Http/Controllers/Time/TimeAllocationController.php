...
class TimeAllocationController extends Controller {
    ...
    private function targetGroups(): array {
        $sources = [
            ...
            'cost_center' => \App\Models\Finance\CostCenter::query()->where('active', true)->orderBy('code')
                ->get(['id', 'code', 'label'])
                ->mapWithKeys(fn (\App\Models\Finance\CostCenter $c): array => [(int) $c->id => trim($c->code . ' — ' . $c->label)]),
            'site' => \App\Models\Facility\Site::query()->orderBy('name')->pluck('name', 'id'),
            // Fahrzeuge haben kein name-Feld: Label + Kennzeichen.
            'vehicle' => \App\Models\Fleet\Vehicle::query()->orderBy('label')->get(['id', 'label', 'license_plate'])
                ->mapWithKeys(fn (\App\Models\Fleet\Vehicle $v): array => [(int) $v->id => trim(($v->label ?? '') . ' ' . ($v->license_plate ?? '')) ?: '#' . $v->id]),
        ...
        $today = now();
        $types = \App\Models\Time\TimeDimensionType::query()
            ->where('enabled', true)
            ->with('values')
            ->orderBy('name')
            ->get();
        foreach ($types as $type) {
            ...
            foreach ($type->values as $value) {
                ...
                $options[] = [
                    'value' => 'dimension:' . Sqid::encode(\App\Models\Time\TimeDimensionValue::class, (int) $value->id),
