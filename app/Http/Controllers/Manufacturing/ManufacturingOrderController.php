...
class ManufacturingOrderController extends Controller {
    ...
    public function show(ManufacturingOrder $order, ManufacturingQualityService $quality): View {
        ...
        return view('manufacturing.show', [
            ...
            'substitutes' => \App\Models\Material\MaterialSubstitute::query()
                ->where('manufacturing_order_id', $order->id)
                ->with(['substituteArticle:id,name', 'plannedArticle:id,name'])
                ->orderByDesc('id')
                ->get(),
            'substituteArticles' => \App\Models\Article\Article::query()->orderBy('name')->limit(500)->get(['id', 'name']),
        ]);
    }
    ...
    public function requestSubstitute(Request $request, ManufacturingOrder $order): RedirectResponse {
        ...
        $articleId = \App\Support\Sqid::decodeOrNumeric(\App\Models\Article\Article::class, (string) $data['substitute_article']);
        $article = $articleId !== null
            ? \App\Models\Article\Article::query()->where('organization_id', $order->organization_id)->find($articleId)
            : null;
        if ($article === null) {
    ...
    public function decideSubstitute(Request $request, ManufacturingOrder $order, \App\Models\Material\MaterialSubstitute $substitute): RedirectResponse {
