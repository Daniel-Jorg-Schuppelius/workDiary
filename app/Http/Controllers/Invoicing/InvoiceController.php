...
class InvoiceController extends Controller {
    ...
    public function store(Request $request, InvoiceGenerator $gen): RedirectResponse {
        ...
        foreach ((array) ($data['excluded_time_entry_ids'] ?? []) as $sqid) {
            $decoded = \App\Support\Sqid::decodeOrNumeric(\App\Models\Time\TimeEntry::class, $sqid);
            if ($decoded !== null) {
    ...
    public function itemForm(Invoice $invoice, ?InvoiceItem $item = null): View {
        ...
        return view('invoices._item_form_dialog', [
            ...
            'articles' => \App\Models\Article\Article::query()->where('sellable', true)->orderBy('name')->limit(500)
    ...
    private function resolveVariant(array $data): ?int {
        ...
        $variant = \App\Models\Article\ArticleVariant::query()->whereKey($variantId)->first();
    ...
    private function articleNumberSnapshot(mixed $articleId, ?int $variantId): ?string {
        if ($variantId !== null) {
            $sku = \App\Models\Article\ArticleVariant::query()->whereKey($variantId)->value('sku');
            if (is_string($sku) && $sku !== '') {
                return mb_substr($sku, 0, 64);
            }
        }
        if ($articleId !== null && $articleId !== '') {
            $number = \App\Models\Article\Article::query()->whereKey((int) $articleId)->value('number');
    ...
    public function deliveriesForm(Invoice $invoice, \App\Services\Invoicing\DeliveryInvoicingService $service): View {
        Gate::authorize('update', $invoice);
        Gate::authorize('viewAny', \App\Models\Manufacturing\ManufacturingOrder::class);
    ...
    public function attachDeliveries(Request $request, Invoice $invoice, \App\Services\Invoicing\DeliveryInvoicingService $service): RedirectResponse {
        ...
        foreach ((array) $data['delivery_ids'] as $raw) {
            $decoded = \App\Support\Sqid::decodeOrNumeric(\App\Models\Inventory\StockDelivery::class, $raw);
            if ($decoded !== null) {
                $ids[] = (int) $decoded;
            }
        }
