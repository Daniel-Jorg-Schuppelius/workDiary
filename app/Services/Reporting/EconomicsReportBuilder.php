...
use App\Models\Gaeb\{BoqItemMapping, BoqItemProgress};
...
class EconomicsReportBuilder {
     ...
     * @return array{revenue:float, cost:float}
     */
    public function expenseAggregate(
        string $from,
        string $to,
        ?int $projectId = null,
        ?int $customerId = null,
    ...
    private function travelAggregate(
        ...
        if ($customer instanceof Customer) {
            $revenue = (float) $this->travelCharges
                ...
                ->sum(static fn(\App\Services\Billing\Travel\TravelCharge $c): float => $c->amount());
        }
