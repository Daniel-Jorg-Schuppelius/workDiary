...
return [
    ...
    'field' => [
        ...
        'jit_hint' => 'Creates a new account on the first IdP login (license limit applies). Never links existing accounts — e-mail collisions are rejected. Only for verified SSO domains and, with OIDC, email addresses confirmed by the IdP.',
        'jit_role' => 'JIT default role',
        'jit_role_none' => 'no role',
        'email_link_hint' => 'On the first SSO login, link an existing account via e-mail (only on exactly one match). Afterwards only issuer + subject counts. Only with an email confirmed by the IdP (OIDC) and a verified SSO domain.',
