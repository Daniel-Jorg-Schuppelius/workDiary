...
| TimeExportLine                     | Kind-Tabelle von `TimeExport` (MVP-019). Zugriff ausschließlich über den mandantengescopten Eltern-Export; Mandantengrenze über `time_exports.organization_id`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| TimeExportEvent                    | Append-only Audit-Child von `TimeExport` (MVP-019). Wird nur über den Eltern-Export gelesen/geschrieben.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| TimeCorrectionItem                 | Kind-Tabelle von `TimeCorrection` (MVP-017). Zugriff ausschließlich über den mandantengescopten Eltern-Korrekturlauf.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| MonthClosureEvent                  | Append-only Audit-Child von `MonthClosure` (MVP-016). Wird nur über die Eltern-Schließung gelesen/geschrieben; Mandantengrenze über `month_closures.organization_id`.                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
...
| UserBookmark                       | Persönliche Lesezeichen pro User (Phase H). Mandantengrenze ergibt sich transitiv über `user_id → users.organization_id`; eigene `organization_id`-Spalte ist redundant. Policy gestattet ausschließlich Zugriff auf eigene Datensätze.                                                                                                                                                                                                                                                                                                                                                                                            |
| UserDashboardWidget                | Persönliche Dashboard-Widget-Konfiguration pro User (Phase G). Mandantengrenze transitiv über `user_id → users.organization_id`; Controller scopt ausschließlich `Auth::id()` und schreibt keine fremden Datensätze.                                                                                                                                                                                                                                                                                                                                                                                                               |
| UserFilterPreset                   | Persönliche Filter-Presets pro User (Folge-Iteration zu Phase H). Mandantengrenze transitiv über `user_id → users.organization_id`; Policy gestattet nur eigene Zugriffe.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| UserQualification                  | Qualifikations-/Unterweisungs-Zuordnung je User (Pivot `user_qualifications`, Feature 013 Arbeitsschutz). Mandantengrenze transitiv über `user_id → users.organization_id`; Zugriff ausschließlich über `$user->qualifications()`. Eigene `organization_id`-Spalte redundant.                                                                                                                                                                                                                                                                                                                                                       |
| Chat\MessageReaction               | Reaktion (Emoji/Like) auf eine `Chat\Message`. Zugriff ausschließlich über `message_id`; Mandantengrenze über das tenant-gescopte Eltern-`Message` (→ `Channel`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| Chat\Poll                          | Umfrage an einer `Chat\Message`. Mandantengrenze transitiv über `message_id → chat_messages.organization_id`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| Chat\PollOption                    | Kind von `Chat\Poll`. Zugriff ausschließlich über `poll_id`; Mandantengrenze über Poll → Message → Channel.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Chat\PollVote                      | Stimme zu einer `Chat\PollOption`. Mandantengrenze transitiv über Option → Poll → Message; zusätzlich an `user_id` gebunden.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| CommunicationNoteParticipant       | Kind-Tabelle von `CommunicationNote` (MVP-012). Zugriff ausschließlich über die tenant-gescopte Eltern-Notiz; Mandantengrenze über `communication_notes.organization_id`.                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| DocumentVersion                    | Append-only Datei-Version eines `Document` (MVP-031). Zugriff ausschließlich über das tenant-gescopte Eltern-Dokument (`documents.organization_id`); Controller bindet Versionen nur in Kombination mit dem Dokument (FK-Check im Download-Pfad). Eigene `organization_id` wäre redundant.                                                                                                                                                                                                                                                                                                                                         |
| KnowledgeArticleLink               | Verknüpfung Artikel ↔ Auftrag/Asset/Kunde/Protokoll (Feature 011). Zugriff ausschließlich über den tenant-gescopten `KnowledgeArticle` (`knowledge_articles.organization_id`); Controller bindet Links nur in Kombination mit dem Artikel (FK-Check in `destroyLink`).                                                                                                                                                                                                                                                                                                                                                             |
| KnowledgeArticleFeedback           | Wertung „hat geholfen" je User und Artikel (Feature 011). Zugriff ausschließlich über den tenant-gescopten `KnowledgeArticle`; zusätzlich an `user_id` gebunden (Unique je Artikel+User). Eigene `organization_id` wäre redundant.                                                                                                                                                                                                                                                                                                                                                                                                 |
| Whistleblowing\CaseEvent           | Append-only Event-Hash-Kette des Hinweisgebermoduls (Abschnitt 9.6/12), analog `OrganizationAuditLog`: nullable `organization_id` **bewusst ohne FK** (Cascade würde die Kette zerreißen, `organization_id` geht in den Hash ein); registriert in `config('audit.chains')` und via `audit:verify` scope-frei über alle Zeilen verifizierbar.                                                                                                                                                                                                                                                                                       |
| Privacy\RequestEvent               | Append-only Event-Hash-Kette für Betroffenenanfragen (Datenschutzmodul), gleiches Muster wie `Whistleblowing\CaseEvent`: nullable `organization_id` ohne FK, minimierte Metadaten ohne Klartext-PII; Kette muss via `audit:verify` ohne Tenant-Scope verifizierbar sein.                                                                                                                                                                                                                                                                                                                                                           |
| Privacy\IncidentEvent              | Append-only Event-Hash-Kette für Datenschutzvorfälle, gleiches Muster wie `Privacy\RequestEvent` (nullable `organization_id` ohne FK, `config('audit.chains')`, `audit:verify` scope-frei).                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| Whistleblowing\CaseTombstone       | Tombstone-Ledger nach Crypto-Shredding eines Falls (Abschnitt 16/25): Minimalnachweis OHNE Meldeinhalte (Fallnummer, Zeitraum, Abschlusskategorie, letzter Audit-Hash). Nullable `organization_id` ohne FK — überlebt Fall-/Org-Löschung und wird beim Backup-Restore erneut angewandt; ein Global-Scope würde diese Funktion aushebeln.                                                                                                                                                                                                                                                                                           |
| PlanModuleGrace                    | Downgrade-/Karenz-Ledger des Lizenz-Layers (`plan_module_grace`). Hat `organization_id` (FK + Unique je Org+Modul), wird aber in Command-/Observer-Kontexten ohne Tenant-Kontext orgübergreifend gelesen (`plans:purge`, `PlanModuleService`) und dort immer explizit nach `organization_id` gefiltert — bewusst ohne Global-Scope.                                                                                                                                                                                                                                                                                                |
| Auth\TwoFactorCredential           | Eingerichteter zweiter Faktor eines Users (TOTP/E-Mail/WebAuthn). Mandantengrenze transitiv über `user_id → users.organization_id` (FK cascade), analog `UserBookmark`; Zugriff ausschließlich über `$user->twoFactorCredentials()` bzw. mit explizitem Ownership-Check (`removeCredential`: `abort_unless(user_id === Auth)`).                                                                                                                                                                                                                                                                                                    |
| Finance\BillingTransferItem        | Quellnachweis-Position eines Übergabenachweises (Feature 045). Kind-Tabelle des tenant-gebundenen `BillingTransfer` — Mandantengrenze transitiv über `billing_transfers.organization_id` (FK cascade), analog `TimeExportLine`; Zugriff ausschließlich über die `items()`-Relation des Transfers. Eigene `organization_id` wäre redundant.                                                                                                                                                                                                                                                                                         |
| Finance\BillingTransferEvent       | Append-only Event-Hash-Kette der Finanzschnittstelle (Feature 045), gleiches Muster wie `Whistleblowing\CaseEvent`: nullable `organization_id` **bewusst ohne FK** (Cascade würde die Kette zerreißen, `organization_id` geht in den Hash ein); registriert in `config('audit.chains')` und via `audit:verify` scope-frei über alle Zeilen verifizierbar; Einträge überdauern die Löschung von Transfer/Organisation.                                                                                                                                                                                                              |
| Finance\PaymentReconciliationEvent | Append-only Event-Hash-Kette des Zahlungsabgleichs (Feature 045, Priorität 3), identisches Muster wie `BillingTransferEvent`: nullable `organization_id` **bewusst ohne FK** (Cascade würde die Kette zerreißen, `organization_id` geht in den Hash ein); registriert in `config('audit.chains')` und via `audit:verify` scope-frei verifizierbar; Einträge überdauern die Löschung von Bankumsatz/Organisation. Der `payload` enthält bewusst keine PII (keine IBANs/Namen/Verwendungszwecke im Klartext).                                                                                                                        |
| Isms\IsmsAuditPackageToken         | Prüfer-Download-Token eines ISMS-Auditpakets (Feature 046, Inkrement E). Kind-Tabelle des tenant-gebundenen `IsmsAuditPackage` — Mandantengrenze transitiv über `isms_audit_packages.organization_id` (FK cascade), analog `ProtocolSignatureToken`. Der öffentliche Prüfer-Download (ohne Login/Org-Session) löst den Token ausschließlich über den persistierten SHA-256-Hash auf (`token_hash`, Klartext wird nie gespeichert); ein Global-Scope würde genau diesen Zugriff aushebeln. Interne Aktionen (Widerruf, Token-Anlage) autorisieren immer über die org-gescopte Paket-Query (`IsmsAuditPackagePolicy::manageTokens`). |
| ExternalParticipantEvent           | Append-only Nachweis externer Aktionen (Feature 033): Zugriff, Kommentar, Upload, Bestätigung eines externen Beteiligten (Subunternehmer/Prüfer/Sachverständiger). Kind-Tabelle des tenant-gebundenen `ExternalParticipant` — Mandantengrenze transitiv über `external_participants.organization_id` (FK cascade), analog `TimeExportLine`. Akteur ist der externe Beteiligte (kein interner `user_id`); der Zugriff erfolgt ausschließlich über die `ExternalParticipant`-Relation. `ExternalParticipant` selbst nutzt regulär `BelongsToOrganization`; nur das Event ist hier geführt. |
| RestoreTest                        | Restore-Test-Register (Feature 017): plattformweites Protokoll durchgeführter Wiederherstellungs-Tests. Analog `BackupHeartbeat` findet der Restore-Vorgang ohne Tenant-Kontext statt; `performed_by_user_id` ist nullable und FK-frei, damit das revisionsrelevante Protokoll eine Nutzer-Löschung überlebt. Zugriff ausschließlich über die plattformweite Admin-Statusseite (Permission `backup.view` / `backup.restoreTest.log`), kein org-gebundener Lesepfad.                                                                                                                                                                  |
