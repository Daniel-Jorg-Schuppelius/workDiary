# Feature-Roadmap

Diese Feature-Roadmap hält Produktideen fest, bevor sie in konkrete GitHub
Issues zerlegt werden. Die Dateien beschreiben das fachliche Ziel, den
Differenzierungswert und den MVP-Umfang. GitHub Issues sollten später auf die
jeweilige Feature-Datei verlinken.

Der entwicklungsnahe Zuschnitt für eine erste Version liegt in
[000 Umsetzungsplan MVP](./000-umsetzungsplan-mvp.md). Dieser Plan bündelt die
Roadmap in Phasen, Epics und konkrete Issue-Vorschläge.

Ein importierbarer GitHub-Issue-Backlog mit Titeln, Labels, Quellen und
Definition-of-Done steht unter
[GitHub-Issue-Backlog MVP](./github-issues-mvp.md).

## Prioritäten

| Priorität | Feature | Status | Warum wichtig |
| --------- | ------- | ------ | ------------- |
| P0 | [Datenschutz, Sicherheit und Datenlebenszyklus](./016-datenschutz-dsgvo-datenlebenszyklus.md) | In Progress | Zentrales Produktversprechen: keine Datenverkäufe, keine versteckte Weitergabe, klare Datenhoheit |
| P0 | [Sicherheitsprüfung und Release-Gate](./051-sicherheitspruefung-release-gate.md) | In Progress | Prüft den vollständigen MVP-Release-Kandidaten einschließlich 2FA, behebt Befunde und sperrt eine ungeprüfte Produktivfreigabe |
| P0 | [Toolkit-Nutzung und Konsolidierung](./052-toolkit-nutzung-konsolidierung.md) | In Progress | Prüft app-lokale Hilfslogik gegen die eigenen Toolkits, beseitigt bestätigte Duplikate und führt fehlende fachneutrale Funktionen im passenden Paket ein |
| P0 | [Mandantenfähigkeit und Betriebsmodelle](./015-mandantenfaehigkeit-betriebsmodelle.md) | Done | Grundlage für lokalen Verkauf, Private-Cloud-Instanzen und SaaS-Betrieb mit sicherer Datenisolation |
| P0 | [Aufzeichnung und Zeiterfassung als Kernprodukt](./001-zeiterfassung-kernprodukt.md) | In Progress | Nachweis, wer wann welche Aufträge angenommen und bearbeitet hat, welche Zeit verbraucht wurde und welche Dienstmittel genutzt wurden |
| P0 | [Auswertungen und Entscheidungsgrundlagen](./002-auswertungen-entscheidungsgrundlagen.md) | Done | Macht aus Aufzeichnungen belastbare Zahlen, Grafiken und Ableitungen zu Kunden, Produkten, Effizienz und Schulungsbedarf |
| P0 | [Dokumentation und Abnahmeprotokolle](./003-dokumentation-abnahmeprotokolle.md) | Done | Sichert Baustellen, Produkte, Anlagen und Aufträge mit Fotos, Checklisten, Unterschriften und Protokollen ab |
| P1 | [Datenschutzmanagement: VVT, AVV und Betroffenenrechte](./043-datenschutzmanagement-vvt-avv-betroffenenrechte.md) | In Progress | Macht Datenschutzpflichten mit Fristen, Verantwortlichen, Verträgen und Nachweisen operativ bearbeitbar |
| P1 | [ISMS und ISO/IEC 27001-Auditbereitschaft](./044-isms-iso-27001-auditbereitschaft.md) | In Progress | Führt Unternehmen von Risiken und Maßnahmen zu belastbaren Nachweisen und einem auditbereiten ISMS |
| P1 | [Zertifizierungsmanagement und integriertes Managementsystem](./046-zertifizierungsmanagement-integriertes-managementsystem.md) | In Progress | Schafft eine gemeinsame, zertifizierungsfreundliche Basis für ISO 27001, 27701, 9001, 22301, 45001, 37301 und 42001 |
| P1 | [Backup, Restore und Disaster Recovery](./017-backup-restore-disaster-recovery.md) | In Progress | Schützt Nachweisdaten, Anhänge, Protokolle und Mandanten vor Datenverlust |
| P1 | [Import, Migration und Onboarding](./020-import-migration-onboarding.md) | In Progress | Senkt Wechselhürden und bringt Kunden schneller produktiv ins System |
| P1 | [Integrationen und offene API](./008-integrationen-api.md) | In Progress | Positioniert WorkDiary als operative Integrationsdrehscheibe mit klarer Datenführerschaft zwischen Faktura, Warenwirtschaft, Buchhaltung, Kalender und eigenen Tools |
| P1 | [Gewerke- und Branchenprofile](./042-gewerke-branchenprofile.md) | In Progress | Erlaubt fachnahe Startkonfigurationen für Elektro, SHK, IT-Service, Bau, Facility und weitere Gewerke |
| P1 | [Tarife, Lizenzportal und Abrechnung](./021-tarife-lizenzportal-abrechnung.md) | In Progress | Macht On-Premise- und SaaS-Vertrieb steuerbar |
| P1 | [Einheitliche Bedienung und UX-Konventionen](./037-einheitliche-bedienung-ux-konventionen.md) | In Progress | Senkt Schulungsaufwand und hält Bedienmuster über alle Module konsistent |
| P1 | [Barrierefreiheit und Zugänglichkeit](./038-barrierefreiheit-zugaenglichkeit.md) | In Progress | Macht Kernflows robuster, zugänglicher und rechtlich zukunftssicher |
| P1 | [Hilfe, Dokumentation und In-App-Unterstützung](./039-hilfe-dokumentation-in-app.md) | In Progress | Reduziert Supportaufwand und erleichtert Einführung und tägliche Nutzung |
| P1 | [Suche, Timeline und Fallakte](./023-suche-timeline-fallakte.md) | In Progress | Bündelt alle Nachweise zu Kunde, Auftrag, Produkt oder Asset an einer Stelle |
| P1 | [Produkt-/Objektakte und Lebenszyklus](./027-produkt-objektakte-lebenszyklus.md) | In Progress | Führt die langfristige Historie von Anlagen, Produkten, Baustellen und Geräten zusammen |
| P1 | [Terminierung, Einsatzplanung und Disposition](./028-terminierung-einsatzplanung-disposition.md) | In Progress | Plant konkrete Aufträge mit Personal, Qualifikation, Dienstmitteln, Standort und SLA |
| P1 | [Kommunikationsprotokoll](./030-kommunikationsprotokoll.md) | Done | Dokumentiert Abstimmungen, Rückfragen und Entscheidungen zu Fällen und Aufträgen |
| P1 | [Hinweisgebersystem und anonyme Meldestelle](../hinweisgebersystem.md) | Done | Ermoeglicht vertrauliche Compliance-Meldungen mit anonymer Zwei-Wege-Kommunikation, Fristen und strikt begrenztem Zugriff |
| P1 | [Dokumentenmanagement](./031-dokumentenmanagement.md) | Done | Verwalten von Verträgen, Prüfberichten, Zertifikaten und Herstellerdokumenten mit Versionen |
| P1 | [Klassifikationen, Tags und Datenqualität](./024-klassifikationen-tags-datenqualitaet.md) | In Progress | Sichert strukturierte Daten für belastbare Auswertungen statt Freitext-Chaos |
| P1 | [Prozeduren, Arbeitsanweisungen und Checklisten](./026-prozeduren-arbeitsanweisungen-checklisten.md) | In Progress | Erzwingt und dokumentiert verbindliche Abläufe, Pflichtnachweise und Vier-Augen-Schritte |
| P1 | [Fertigungs-, Montage- und Arbeitsaufträge](./047-fertigungs-montage-arbeitsauftraege.md) | Planned | Verbindet versionierte Arbeitspläne mit Sollmengen, Rezepturen, Materialbedarf, Wartezeiten und dokumentierten Fertigungsergebnissen |
| P1 | [Lagerwirtschaft und Bestandsintegration](./048-lagerwirtschaft-bestandsintegration.md) | Planned | Führt Bestände bei Lexoffice-Nutzung lokal oder bindet eine externe Warenwirtschaft als führenden Bestandsprovider an |
| P1 | [GAEB-Leistungsverzeichnisse und AVA-Austausch](./049-gaeb-leistungsverzeichnisse.md) | Done | Verbindet Bau-/Ausbauprojekte mit Leistungsverzeichnissen, Ordnungszahlen, Aufmaß, Nachträgen und GAEB-Import/-Export |
| P1 | [Lieferantenkataloge, Shopimport und Preisabgleich](./050-lieferantenkataloge-shopimport-preisabgleich.md) | Done | Übernimmt Shop- und Katalogartikel kontrolliert in den Artikelstamm, erkennt Preisänderungen und sichert Margen |
| P1 | [Offline-Sync und Konfliktlösung](./035-offline-sync-konfliktloesung.md) | Proposed | Verhindert Datenverlust und stille Überschreibungen bei mobiler Offline-Erfassung |
| P1 | [Mobiler Field-Workflow](./004-mobiler-field-workflow.md) | In Progress | Macht den Nachweis für Außendienst, Service, Handwerk und Einsatzteams direkt am Ort der Arbeit erfassbar |
| P1 | [Lohn, Zuschläge und DATEV/Lexware](./005-lohn-zuschlaege-datev-lexware.md) | In Progress | Macht Zeiten abrechnungs- und payrollfähig statt nur dokumentiert |
| P1 | [DATEV- und Finanzschnittstelle](./045-datev-finanzschnittstelle.md) | In Progress | Übergibt Zeiten und Produkte getrennt an DATEV Faktura und verbindet DATEV-Dateien, Zahlungsabgleich sowie weitere Finanzformate |
| P1 | [Inventar, Dienstmittel und Assets](./009-inventar-dienstmittel-assets.md) | In Progress | Macht genutzte Geräte, Werkzeuge, Maschinen, Fahrzeuge und Produkte nachvollziehbar |
| P1 | [SLA, Verträge und Service-Level](./010-sla-vertraege-servicelevel.md) | In Progress | Bewertet Aufwand gegen Vertragsziele, Reaktionszeiten und Wartungspflichten |
| P1 | [Nachkalkulation und Wirtschaftlichkeit](./014-nachkalkulation-wirtschaftlichkeit.md) | In Progress | Zeigt, ob Kunden, Produkte, Aufträge und Leistungen wirtschaftlich tragfähig sind |
| P2 | [Benachrichtigungen und Eskalationen](./018-benachrichtigungen-eskalationen.md) | Done | Meldet Risiken, Fristen, offene Nachweise und kritische Betriebsereignisse rechtzeitig |
| P2 | [Demo-, Testdaten und Musterbranchen](./040-demo-testdaten-musterbranchen.md) | In Progress | Macht Vertrieb, Tests und Onboarding mit realistischen Beispielen reproduzierbar |
| P2 | [Support und Fehlerdiagnose für Kundeninstallationen](./041-support-fehlerdiagnose-kundeninstallationen.md) | In Progress | Erleichtert datenschutzfreundliche Diagnose für SaaS, Private Cloud und On-Premise |
| P2 | [Karten, Standort und Leitstelle](./029-karten-standort-leitstelle.md) | In Progress | Macht Einsätze, Kunden, Objekte, Touren und SLA-Risiken räumlich sichtbar |
| P2 | [Rollen, Rechte und Produktprofile](./019-rollen-rechte-produktprofile.md) | Done | Macht Rechteverwaltung für Kunden verständlich und sicher voreingestellt |
| P2 | [Release-, Update- und Plugin-Strategie](./022-release-update-plugin-strategie.md) | In Progress | Sichert Updates, Plugins und lokale Installationen im Betrieb ab |
| P2 | [Vorlagen- und Formularsystem](./032-vorlagen-formularsystem.md) | Done | Vereinheitlicht Protokolle, Checklisten, PDFs und Formulare über wiederverwendbare Vorlagen |
| P2 | [Externe Beteiligte, Subunternehmer und Prüfer](./033-externe-beteiligte-subunternehmer-pruefer.md) | In Progress | Bindet externe Akteure mit begrenzten Rechten und nachvollziehbaren Aktionen ein |
| P2 | [Internationalisierung und Rechtsräume](./034-internationalisierung-rechtsraeume.md) | In Progress | Bereitet Sprache, Feiertage, Spesen, Währungen und Rechtsregeln für DACH/EU vor |
| P2 | [Compliance, Korrekturen und Audit](./006-compliance-korrekturen-audit.md) | In Progress | Vertrauensanker für ArbZG, DSGVO, Prüfungen und interne Freigaben |
| P2 | [Dienstplan-Intelligenz](./007-dienstplan-intelligenz.md) | In Progress | Hebt WorkDiary von einfacher Planung zu Workforce-Steuerung |
| P2 | [Wissensbasis und Problemhistorie](./011-wissensbasis-problemhistorie.md) | Done | Wandelt wiederkehrende Probleme und Lösungen in nutzbares Betriebswissen um |
| P2 | [Kundenportal und Freigaben](./012-kundenportal-freigaben.md) | In Progress | Ermöglicht externe Abnahmen, Rückfragen und Dokumentenfreigaben mit Nachweis |
| P2 | [Qualität, Sicherheit und Arbeitsschutz](./013-qualitaet-sicherheit-arbeitsschutz.md) | In Progress | Verankert Unterweisungen, PSA, Prüfpflichten und Sicherheitsereignisse im Arbeitsnachweis |
| P3 | [KI-Assistenz und intelligente Automatisierung](./025-ki-assistenz-automatisierung.md) | Proposed | Ermöglicht spätere Assistenzfunktionen ohne Verlust von Datenschutz und Datenhoheit |
| P3 | [Produktmetriken und Betriebsmetriken](./036-produktmetriken-betriebsmetriken.md) | Done | Erlaubt sicheren Betrieb ohne verdecktes Tracking oder Verwertung von Kundendaten |

## Statuswerte

- `Proposed`: fachlich beschrieben, noch nicht als Umsetzung geplant.
- `Planned`: priorisiert und in GitHub Issues zerlegt.
- `In Progress`: Umsetzung läuft.
- `Done`: MVP ist umgesetzt und dokumentiert.

## Arbeitsweise

1. Feature-Datei fachlich schärfen.
2. MVP in kleine GitHub Issues zerlegen.
3. Issue-Links in der Feature-Datei nachtragen.
4. Nach Umsetzung Status und Dokumentation aktualisieren.
